...
- **Decoupled Viewer vs Configurator**: The 3D Viewer is a standalone core component capable of rendering models entirely independently via a simple API URL. The Configurator is an *extended overlay feature* that seamlessly wraps the Viewer. Users can browse and view models instantly; backend CAD generation is only triggered if they interact with the optional Configurator UI.
- **Static Asset Delivery & Tenant Isolation**: The Backend Orchestrator *never* serves `.glb` binaries directly. It is an anti-pattern that blocks the async event loop. Instead, `GET /api/models/{id}` returns a **Signed URL**. In production, this points to an aggressively cached **AWS CloudFront CDN**. The CDN guarantees blazing-fast delivery for the Viewer, while the Signed URL cryptographically enforces tenant isolation to prevent unauthorized model scraping.
